#!/usr/bin/env bash
# Phase B-5 migrator — detect-first, one commit per module.
...
#   5. Pass → one commit "refactor(phase-b5): migrate <slug> module with compat shims".
...
# B-5 modules in execution order:
#   barcode first    (pure presentation + lib files; no platform writes),
#   superadmin last  (platform-level writes to shops/plans/payments — slightly higher risk).
MODULES="
barcode|Barcode
...
shim_body() {
  ...
  case "$KIND" in
    named)
      printf "// Phase B-5 compat shim. Real file lives at %s.\nexport * from '%s';\n" "$TGT" "$TGT" ;;
    default)
      printf "// Phase B-5 compat shim. Real file lives at %s.\nexport { default } from '%s';\n" "$TGT" "$TGT" ;;
    default_named)
      printf "// Phase B-5 compat shim. Real file lives at %s.\nexport { default } from '%s';\nexport * from '%s';\n" "$TGT" "$TGT" "$TGT" ;;
    css)
      printf "/* Phase B-5 compat shim. Real file lives at %s.css */\n@import '%s.css';\n" "$TGT" "$TGT" ;;
  esac
}
...
migrate_module() {
  ...
  git commit -m "refactor(phase-b5): migrate $SLUG module with compat shims" --quiet
...
echo "Phase B-5 — $SHIM_MODE shims"
