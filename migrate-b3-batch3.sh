#!/usr/bin/env bash
# Phase B-3 Batch 3 migrator — detect-first, one commit per module.
...
#   5. Pass → one commit "refactor(phase-b3-batch3): migrate <slug> module with compat shims".
...
# Batch-3 modules in execution order:
#   vendors first (purchases depends on vendor master),
#   karigar + bullion next (standalone subsystems),
#   purchases last (largest module, depends on vendors via shim).
MODULES="
vendors|Vendors
karigar|Karigar
bullion|Bullion
...
migrate_module() {
  ...
  git commit -m "refactor(phase-b3-batch3): migrate $SLUG module with compat shims" --quiet
...
echo "Phase B-3 Batch 3 — $SHIM_MODE shims"
