#!/usr/bin/env bash
# Phase B-3 Batch 3 detector — read-only.
# Mac bash 3.2 compatible: no mapfile/readarray, no ${var,,}/${var^^}, no associative arrays.
# Modules: vendors, karigar, bullion, purchases (procurement chain)
#
# Exit 0 if no blockers in batch-3 modules, 1 otherwise.
...
purchases|Purchases
"
# (Batch 1+2 modules included read-only so the detector reports unified status;
# blockers in earlier batches will NOT halt this script — only batch-3 blockers do.)

BATCH3_SET=" vendors karigar bullion purchases "
...
echo "$MODULES" | while IFS='|' read SLUG NAME; do
  ...
  case " $BATCH3_SET " in *" $SLUG "*) IS_B3=yes ;; *) IS_B3=no ;; esac
  TAG="(batch-1)"
  [ "$IS_B3" = "yes" ] && TAG="(BATCH-3)"
  ...
  if [ -n "$PAGE_OLD" ] || [ -n "$PAGE_NEW_ACTUAL" ]; then
    ...
    case "$STATE" in
      DONE|ABSENT) : ;;
      TODO_*|MOVED_NO_SHIM) [ "$IS_B3" = "yes" ] && ALL_TODOS=$((ALL_TODOS+1)) ;;
      *) [ "$IS_B3" = "yes" ] && ALL_BLOCKERS=$((ALL_BLOCKERS+1)) ;;
    esac
  else
  ...
  if [ -z "$COMP_LIST" ]; then
    ...
    while IFS= read f; do
      ...
      case "$STATE" in
        DONE|ABSENT) : ;;
        TODO_*|MOVED_NO_SHIM) [ "$IS_B3" = "yes" ] && echo "TODO" || echo "B1_NOOP" ;;
        *) [ "$IS_B3" = "yes" ] && echo "BLOCKER" || echo "B1_NOOP" ;;
      esac
    done <<<"$COMP_LIST" > /tmp/.b3b3_states.$$
    while IFS= read line; do
      case "$line" in
        TODO)    ALL_TODOS=$((ALL_TODOS+1)) ;;
        BLOCKER) ALL_BLOCKERS=$((ALL_BLOCKERS+1)) ;;
      esac
    done < /tmp/.b3b3_states.$$
    rm -f /tmp/.b3b3_states.$$
  fi
done
...
while IFS='|' read SLUG NAME; do
  [ -z "$SLUG" ] && continue
  case " $BATCH3_SET " in *" $SLUG "*) ;; *) continue ;; esac
...
if [ $ALL_BLOCKERS -gt 0 ]; then
  echo "Resolve Batch-3 blockers (SPLIT_BRAIN, BOTH_SHIMS, STUB_NO_OLD, OLD_IS_SHIM_NO_NEW) manually before running the migrator."
  exit 1
fi
echo "No batch-3 blockers. Run migrate-b3-batch3.sh to process the $ALL_TODOS todo(s)."
exit 0
