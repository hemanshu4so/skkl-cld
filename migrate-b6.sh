#!/usr/bin/env bash
# Phase B-6 migrator — detect-first, one commit per module.
# Modules: printing (template designer + PrintRenderer + printTemplate.js).
...
#   5. Pass → one commit "refactor(phase-b6): migrate <slug> module with compat shims".
...
# B-6 module: printing (single).
# Files included in the one commit:
#   - src/pages/PrintDesigner.jsx          → src/modules/printing/pages/PrintDesigner.jsx
#   - src/components/PrintRenderer.jsx     → src/modules/printing/components/PrintRenderer.jsx
#   - src/lib/printTemplate.js             → src/modules/printing/lib/printTemplate.js
MODULES="
printing|Printing
"
...
shim_body() {
  ...
  case "$KIND" in
    named)
      printf "// Phase B-6 compat shim. Real file lives at %s.\nexport * from '%s';\n" "$TGT" "$TGT" ;;
    default)
      printf "// Phase B-6 compat shim. Real file lives at %s.\nexport { default } from '%s';\n" "$TGT" "$TGT" ;;
    default_named)
      printf "// Phase B-6 compat shim. Real file lives at %s.\nexport { default } from '%s';\nexport * from '%s';\n" "$TGT" "$TGT" "$TGT" ;;
    css)
      printf "/* Phase B-6 compat shim. Real file lives at %s.css */\n@import '%s.css';\n" "$TGT" "$TGT" ;;
  esac
}
...
migrate_module() {
  ...
  git commit -m "refactor(phase-b6): migrate $SLUG module with compat shims" --quiet
...
echo "Phase B-6 — $SHIM_MODE shims"
