#!/usr/bin/env bash
# Phase B-4 migrator — detect-first, one commit per module.
...
#   5. Pass → one commit "refactor(phase-b4): migrate <slug> module with compat shims".
...
# B-4 modules in execution order:
#   reports first  (read-only — smallest blast radius),
#   accounting next (aggregates dayBookEntries written by other modules — read-heavy from its own POV),
#   schemes last   (most complex: 11+1 cron, money flow, customerTransactions writes).
MODULES="
reports|Reports
...
shim_body() {
  ...
  case "$KIND" in
    named)
      printf "// Phase B-4 compat shim. Real file lives at %s.\nexport * from '%s';\n" "$TGT" "$TGT" ;;
    default)
      printf "// Phase B-4 compat shim. Real file lives at %s.\nexport { default } from '%s';\n" "$TGT" "$TGT" ;;
    default_named)
      printf "// Phase B-4 compat shim. Real file lives at %s.\nexport { default } from '%s';\nexport * from '%s';\n" "$TGT" "$TGT" "$TGT" ;;
    css)
      printf "/* Phase B-4 compat shim. Real file lives at %s.css */\n@import '%s.css';\n" "$TGT" "$TGT" ;;
  esac
}
...
migrate_module() {
  ...
  git commit -m "refactor(phase-b4): migrate $SLUG module with compat shims" --quiet
...
echo "Phase B-4 — $SHIM_MODE shims"
