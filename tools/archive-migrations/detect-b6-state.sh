...
#
# Exit 0 if no blockers in B-6 modules, 1 otherwise.
...
# blockers in earlier batches will NOT halt this script — only B-6 blockers do.)
...
echo "$MODULES" | while IFS='|' read SLUG NAME; do
  ...
  case " $B6_SET " in *" $SLUG "*) IS_B6=yes ;; *) IS_B6=no ;; esac
  TAG="(batch-1)"
  [ "$IS_B6" = "yes" ] && TAG="(B-6)"
  ...
  if [ -n "$PAGE_OLD" ] || [ -n "$PAGE_NEW_ACTUAL" ]; then
    ...
    case "$STATE" in
      DONE|ABSENT) : ;;
      TODO_*|MOVED_NO_SHIM) [ "$IS_B6" = "yes" ] && ALL_TODOS=$((ALL_TODOS+1)) ;;
      *) [ "$IS_B6" = "yes" ] && ALL_BLOCKERS=$((ALL_BLOCKERS+1)) ;;
    esac
  else
  ...
  if [ -z "$COMP_LIST" ]; then
    ...
    while IFS= read f; do
      ...
      case "$STATE" in
        DONE|ABSENT) : ;;
        TODO_*|MOVED_NO_SHIM) [ "$IS_B6" = "yes" ] && echo "TODO" || echo "B1_NOOP" ;;
        *) [ "$IS_B6" = "yes" ] && echo "BLOCKER" || echo "B1_NOOP" ;;
      esac
    done <<<"$COMP_LIST" > /tmp/.b6_states.$$
...
while IFS='|' read SLUG NAME; do
  [ -z "$SLUG" ] && continue
  case " $B6_SET " in *" $SLUG "*) ;; *) continue ;; esac
...
if [ $ALL_BLOCKERS -gt 0 ]; then
  echo "Resolve B-6 blockers (SPLIT_BRAIN, BOTH_SHIMS, STUB_NO_OLD, OLD_IS_SHIM_NO_NEW) manually before running the migrator."
  exit 1
fi
echo "No B-6 blockers. Run migrate-b6.sh to process the $ALL_TODOS todo(s)."
exit 0
