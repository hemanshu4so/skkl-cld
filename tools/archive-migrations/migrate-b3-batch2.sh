#!/usr/bin/env bash
# Phase B-3 Batch 2 migrator — detect-first, one commit per module.
...
#   5. Pass → one commit "refactor(phase-b3-batch2): migrate <slug> module with compat shims".
...
# Batch-2 modules in execution order (deliberately: rates → repairs → orders → vouchers)
MODULES="
rates|Rates
repairs|Repairs
orders|Orders
...
migrate_module() {
  ...
  git commit -m "refactor(phase-b3-batch2): migrate $SLUG module with compat shims" --quiet
...
echo "Phase B-3 Batch 2 — $SHIM_MODE shims"
