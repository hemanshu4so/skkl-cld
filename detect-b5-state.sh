#!/usr/bin/env bash
# Phase B-5 detector — read-only.
# Mac bash 3.2 compatible: no mapfile/readarray, no ${var,,}/${var^^}, no associative arrays.
# Modules: barcode, superadmin (isolated subsystems)
#
# Exit 0 if no blockers in B-5 modules, 1 otherwise.
...
# blockers in earlier batches will NOT halt this script — only B-5 blockers do.)

B5_SET=" barcode superadmin "
...
echo "$MODULES" | while IFS='|' read SLUG NAME; do
  ...
  case " $B5_SET " in *" $SLUG "*) IS_B5=yes ;; *) IS_B5=no ;; esac
  TAG="(batch-1)"
  [ "$IS_B5" = "yes" ] && TAG="(B-5)"
  ...
  if [ -n "$PAGE_OLD" ] || [ -n "$PAGE_NEW_ACTUAL" ]; then
    ...
    case "$STATE" in
      DONE|ABSENT) : ;;
      TODO_*|MOVED_NO_SHIM) [ "$IS_B5" = "yes" ] && ALL_TODOS=$((ALL_TODOS+1)) ;;
      *) [ "$IS_B5" = "yes" ] && ALL_BLOCKERS=$((ALL_BLOCKERS+1)) ;;
    esac
  else
  ...
  if [ -z "$COMP_LIST" ]; then
    ...
    while IFS= read f; do
      ...
      case "$STATE" in
        DONE|ABSENT) : ;;
        TODO_*|MOVED_NO_SHIM) [ "$IS_B5" = "yes" ] && echo "TODO" || echo "B1_NOOP" ;;
        *) [ "$IS_B5" = "yes" ] && echo "BLOCKER" || echo "B1_NOOP" ;;
      esac
    done <<<"$COMP_LIST" > /tmp/.b5_states.$$
...
while IFS='|' read SLUG NAME; do
  [ -z "$SLUG" ] && continue
  case " $B5_SET " in *" $SLUG "*) ;; *) continue ;; esac
...
if [ $ALL_BLOCKERS -gt 0 ]; then
  echo "Resolve B-5 blockers (SPLIT_BRAIN, BOTH_SHIMS, STUB_NO_OLD, OLD_IS_SHIM_NO_NEW) manually before running the migrator."
  exit 1
fi
echo "No B-5 blockers. Run migrate-b5.sh to process the $ALL_TODOS todo(s)."
exit 0
