#!/usr/bin/env bash
# Phase B-4 detector — read-only.
# Mac bash 3.2 compatible: no mapfile/readarray, no ${var,,}/${var^^}, no associative arrays.
# Modules: reports, accounting, schemes (financial backbone)
#
# Exit 0 if no blockers in B-4 modules, 1 otherwise.
...
# blockers in earlier batches will NOT halt this script — only B-4 blockers do.)

B4_SET=" reports accounting schemes "
...
echo "$MODULES" | while IFS='|' read SLUG NAME; do
  ...
  case " $B4_SET " in *" $SLUG "*) IS_B4=yes ;; *) IS_B4=no ;; esac
  TAG="(batch-1)"
  [ "$IS_B4" = "yes" ] && TAG="(B-4)"
  ...
  if [ -n "$PAGE_OLD" ] || [ -n "$PAGE_NEW_ACTUAL" ]; then
    ...
    case "$STATE" in
      DONE|ABSENT) : ;;
      TODO_*|MOVED_NO_SHIM) [ "$IS_B4" = "yes" ] && ALL_TODOS=$((ALL_TODOS+1)) ;;
      *) [ "$IS_B4" = "yes" ] && ALL_BLOCKERS=$((ALL_BLOCKERS+1)) ;;
    esac
  else
  ...
  if [ -z "$COMP_LIST" ]; then
    ...
    while IFS= read f; do
      ...
      case "$STATE" in
        DONE|ABSENT) : ;;
        TODO_*|MOVED_NO_SHIM) [ "$IS_B4" = "yes" ] && echo "TODO" || echo "B1_NOOP" ;;
        *) [ "$IS_B4" = "yes" ] && echo "BLOCKER" || echo "B1_NOOP" ;;
      esac
    done <<<"$COMP_LIST" > /tmp/.b4_states.$$
...
while IFS='|' read SLUG NAME; do
  [ -z "$SLUG" ] && continue
  case " $B4_SET " in *" $SLUG "*) ;; *) continue ;; esac
...
if [ $ALL_BLOCKERS -gt 0 ]; then
  echo "Resolve B-4 blockers (SPLIT_BRAIN, BOTH_SHIMS, STUB_NO_OLD, OLD_IS_SHIM_NO_NEW) manually before running the migrator."
  exit 1
fi
echo "No B-4 blockers. Run migrate-b4.sh to process the $ALL_TODOS todo(s)."
exit 0
