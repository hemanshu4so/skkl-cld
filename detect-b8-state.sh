...
#
# Exit 0 if no blockers in B-8 modules, 1 otherwise.
...
# blockers in earlier batches will NOT halt this script — only B-8 blockers do.)
...
echo "$MODULES" | while IFS='|' read SLUG NAME; do
  ...
  case " $B8_SET " in *" $SLUG "*) IS_B8=yes ;; *) IS_B8=no ;; esac
  TAG="(batch-1)"
  [ "$IS_B8" = "yes" ] && TAG="(B-8)"
  ...
  if [ -n "$PAGE_OLD" ] || [ -n "$PAGE_NEW_ACTUAL" ]; then
    ...
    case "$STATE" in
      DONE|ABSENT) : ;;
      TODO_*|MOVED_NO_SHIM) [ "$IS_B8" = "yes" ] && ALL_TODOS=$((ALL_TODOS+1)) ;;
      *) [ "$IS_B8" = "yes" ] && ALL_BLOCKERS=$((ALL_BLOCKERS+1)) ;;
    esac
  else
  ...
  if [ -z "$COMP_LIST" ]; then
    ...
    while IFS= read f; do
      ...
      case "$STATE" in
        DONE|ABSENT) : ;;
        TODO_*|MOVED_NO_SHIM) [ "$IS_B8" = "yes" ] && echo "TODO" || echo "B1_NOOP" ;;
        *) [ "$IS_B8" = "yes" ] && echo "BLOCKER" || echo "B1_NOOP" ;;
      esac
...
while IFS='|' read SLUG NAME; do
  [ -z "$SLUG" ] && continue
  case " $B8_SET " in *" $SLUG "*) ;; *) continue ;; esac
...
if [ $ALL_BLOCKERS -gt 0 ]; then
  echo "Resolve B-8 blockers (SPLIT_BRAIN, BOTH_SHIMS, STUB_NO_OLD, OLD_IS_SHIM_NO_NEW) manually before running the migrator."
  exit 1
fi
echo "No B-8 blockers. Run migrate-b8.sh to process the $ALL_TODOS todo(s)."
exit 0
