#!/usr/bin/env bash
# Phase B-7 migrator — detect-first, one commit per module.
...
#   5. Pass → one commit "refactor(phase-b7): migrate <slug> module with compat shims".
...
# B-7 module: inventory (single, high blast radius).
# Run the dependency analyzer first (analyze-b7-dependency.sh) to map consumers.
# Typical files included in the one commit (detected dynamically):
#   - src/pages/Inventory.jsx              → src/modules/inventory/pages/Inventory.jsx
#   - src/components/inventory/*           → src/modules/inventory/components/*
#   - src/lib/inventoryUtils.js (if any)   → src/modules/inventory/lib/inventoryUtils.js
#   - top-level inventory components       → src/modules/inventory/components/
MODULES="
inventory|Inventory
"
...
shim_body() {
  ...
  case "$KIND" in
    named)
      printf "// Phase B-7 compat shim. Real file lives at %s.\nexport * from '%s';\n" "$TGT" "$TGT" ;;
    default)
      printf "// Phase B-7 compat shim. Real file lives at %s.\nexport { default } from '%s';\n" "$TGT" "$TGT" ;;
    default_named)
      printf "// Phase B-7 compat shim. Real file lives at %s.\nexport { default } from '%s';\nexport * from '%s';\n" "$TGT" "$TGT" "$TGT" ;;
    css)
      printf "/* Phase B-7 compat shim. Real file lives at %s.css */\n@import '%s.css';\n" "$TGT" "$TGT" ;;
  esac
}
...
migrate_module() {
  ...
  git commit -m "refactor(phase-b7): migrate $SLUG module with compat shims" --quiet
...
echo "Phase B-7 — $SHIM_MODE shims"
